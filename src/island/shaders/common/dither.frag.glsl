...
vec4 dither(float color, float intensity) {
    ...
    float bias = clamp(max(length(dFdx(vPosition)), length(dFdy(vPosition))) * 30.0, 0.0, 1.0);
    float level = pow(2.0, 12.0 - floor((bias * 5.0) + 1.0));
    vec2 vnoise = rand(vPosition * level * 0.0625);
    float noise = mix(vnoise[0], vnoise[1], 1.0 - fract((bias * 5.0) + 1.0));
